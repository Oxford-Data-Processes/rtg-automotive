...
def get_glue_schemas_path() -> str:
    current_dir = os.path.dirname(os.path.abspath(__file__))
    glue_schemas_path = os.path.join(current_dir, "models", "glue_schemas.json")
    return glue_schemas_path


glue_schemas_path = get_glue_schemas_path()
...
schemas = load_schemas(glue_schemas_path)
...
def get_db() -> Generator[Session, None, None]:
    ...
    db = create_session(database_name, workgroup, output_bucket)
    try:
        yield db
    finally:
        db.close()
...
async def read_items(
    ...
    Processed = create_model_class(
        "processed", schemas["processed"] + schemas["partition_keys"]
    )
